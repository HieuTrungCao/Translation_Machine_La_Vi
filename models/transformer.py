...
from modules.prototypes import Encoder, Decoder
...
from datasets.cleandata import CleanData
...
class Transformer(nn.Module):
    ...
    def __init__(self, mode=None, model_dir=None, config=None):
        ...
        if "train_data_location" in opt or "train_data_location" in opt.get("data", {}):
            ...
            self.loader = DefaultLoader(
                data_opt["train_data_location"] + "_clean",
                eval_path=data_opt.get("eval_data_location", None) + "_clean",
            ...
            self.loader = MultiLoader(
                opt["data"]["train"], valid=opt["data"].get("valid", None), option=opt
            )
        # input fields

        self.SRC, self.TRG = self.loader.build_field(
            lower=opt.get("lowercase", const.DEFAULT_LOWERCASE)
        )
        ...
        if mode == "train":
            ...
            self.train_iter, self.valid_iter = self.loader.create_iterator(
                self.fields, model_path=model_dir
            )
        elif mode == "eval":
        ...
        train_ignore_length = self.config.get(
            "train_max_length", const.DEFAULT_TRAIN_MAX_LENGTH
        )
        input_max_length = self.config.get(
            "input_max_length", const.DEFAULT_INPUT_MAX_LENGTH
        )
        ...
        decode_strategy_class = strategies[
            opt.get("decode_strategy", const.DEFAULT_DECODE_STRATEGY)
        ]
        decode_strategy_kwargs = opt.get(
            "decode_strategy_kwargs", const.DEFAULT_STRATEGY_KWARGS
        )
        self.decode_strategy = decode_strategy_class(
            self, infer_max_length, self.device, **decode_strategy_kwargs
        )
    ...
    def load_checkpoint(self, model_dir, checkpoint=None, checkpoint_idx=0):
        ...
        if checkpoint is not None:
            ...
            if model_dir is not None:
                ...
                if checkpoint_idx > 0:
                    print(
                        "Found model with index {:d} already saved.".format(
                            checkpoint_idx
                        )
                    )
                    saver.load_model_from_path(
                        self, model_dir, checkpoint_idx=checkpoint_idx
                    )
                else:
    ...
    def forward(self, src, trg, src_mask, trg_mask, output_attention=False):
        ...
        d_output, attn = self.decoder(
            trg, e_outputs, src_mask, trg_mask, output_attention=True
        )
    ...
    def train_step(self, optimizer, batch, criterion):
        ...
        src_mask, trg_mask = create_masks(
            src, trg_input, src_pad, trg_pad, opt.get("device", const.DEFAULT_DEVICE)
        )
    ...
    def validate(self, valid_iter, criterion, maximum_length=None):
        ...
        with torch.no_grad():
            ...
            for batch in valid_iter:
                ...
                src = batch.src.transpose(0, 1).to(
                    opt.get("device", const.DEFAULT_DEVICE)
                )
                trg = batch.trg.transpose(0, 1).to(
                    opt.get("device", const.DEFAULT_DEVICE)
                )
    ...
    def translate_sentence(
        self, sentence, device=None, k=None, max_len=None, debug=False
    ):
        """
        ...
        self.eval()
        if device is None:
            device = self.config.get("device", const.DEFAULT_DEVICE)
        if k is None:
    ...
    def translate_batch_sentence(
        ...
        if batch_size is None:
            batch_size = self.config.get(
                "eval_batch_size", const.DEFAULT_EVAL_BATCH_SIZE
            )
        input_max_length = self.config.get(
            "input_max_length", const.DEFAULT_INPUT_MAX_LENGTH
        )
        ...
        for b_idx in range(0, len(sentences), batch_size):
            ...
            trans_batch = self.translate_batch(
                batch,
                trg_lang=trg_lang,
    ...
    def translate_batch(
        ...
        translated_batch = self.decode_strategy.translate_batch(
            batch_sentences,
            trg_lang=trg_lang,
        ...
        return (
            self.loader.detokenize(translated_batch)
            if not output_tokens
            else translated_batch
        )
    ...
    def run_train(self, model_dir=None, config=None):
        ...
        logging.info(
            "%s * src vocab size = %s"
            % (self.loader._language_tuple[0], len(self.SRC.vocab))
        )
        logging.info(
            "%s * tgt vocab size = %s"
            % (self.loader._language_tuple[1], len(self.TRG.vocab))
        )
        ...
        optimizer = ScheduledOptim(
            optimizer=optimizers.get(optim_algo)(
        ...
        criterion = LabelSmoothingLoss(
            len(self.TRG.vocab), padding_idx=trg_pad, smoothing=opt["label_smoothing"]
        )
        ...
        logging.info(self)
        model_encoder_parameters = filter(
            lambda p: p.requires_grad, self.encoder.parameters()
        )
        model_decoder_parameters = filter(
        ...
        logging.info(
            "Starting training on %s" % (opt.get("device", const.DEFAULT_DEVICE))
        )
        ...
        for epoch in range(checkpoint_idx, opt["epochs"]):
            ...
            valid_loss = self.validate(
                ...
                criterion,
                maximum_length=(
                    self.encoder._max_seq_length,
                    self.decoder._max_seq_length,
                ),
            )
            if (epoch + 1) % opt[
                "save_checkpoint_epochs"
            ] == 0 and model_dir is not None:
                ...
                saver.save_and_clear_model(
                    ...
                    maximum_saved_model=opt.get(
                        "maximum_saved_model_train", const.DEFAULT_NUM_KEEP_MODEL_TRAIN
                    ),
                )
                ...
                best_model_score = saver.save_model_best_to_path(
                    ...
                    bleuscore,
                    maximum_saved_model=opt.get(
                        "maximum_saved_model_eval", const.DEFAULT_NUM_KEEP_MODEL_TRAIN
                    ),
                )
                ...
                logging.info(
                    "epoch: {:03d} - iter: {:05d} - valid loss: {:.4f} - validation time: {:.4f}".format(
                        epoch, i, valid_loss, time.time() - s
                    )
                )
    ...
    def to_logits(
        self, inputs
    ):  # function to include the logits. TODO use this in inference fns as well
    ...
    def prepare_serve(self, serve_path, model_dir=None, check_trace=True, **kwargs):
        ...
        sample_src, sample_trg = torch.randint(
            fake_range, (fake_batch, fake_srclen), dtype=torch.long
        ), torch.randint(fake_range, (fake_batch, fake_trglen), dtype=torch.long)
        sample_src_mask, sample_trg_mask = (
            torch.rand(fake_batch, 1, fake_srclen) > 0.5,
            torch.rand(fake_batch, fake_trglen, fake_trglen) > 0.5,
        )
        sample_src, sample_trg, sample_src_mask, sample_trg_mask = [
            t.to(self.device)
            for t in [sample_src, sample_trg, sample_src_mask, sample_trg_mask]
        ]
        sample_encoded = self.encode(sample_src, sample_src_mask)
        sample_before_logits = self.decode(
            sample_trg, sample_encoded, sample_src_mask, sample_trg_mask
        )
        # bundle within dictionary
        needed_fn = {
            "forward": (sample_src, sample_trg, sample_src_mask, sample_trg_mask),
