...
class DefaultLoader:
    ...
    def _train_path_is_name(self):
        return os.path.isfile(
            self._train_path + self._language_tuple[0]
        ) and os.path.isfile(self._train_path + self._language_tuple[1])
    ...
    def build_field(self, **kwargs):
        ...
        return Field(tokenize=self.lo_tokenize, **kwargs), Field(
            tokenize=self.vi_tokenize,
            init_token=const.DEFAULT_SOS,
            eos_token=const.DEFAULT_EOS,
            **kwargs
        )
    ...
    def build_vocab(self, fields, model_path=None, data=None, **kwargs):
        ...
        if model_path is None or not load_vocab_from_path(
            model_path, self._language_tuple, fields
        ):
            ...
            if data is not None:
                ...
                external_vocab_location = self._option.get(
                    "external_vocab_location", None
                )
                ...
                with io.open(vocab_src, "r", encoding="utf-8") as svf:
                    ...
                    src_field.vocab = src_field.vocab_cls(
                        mock_counter, specials=special_tokens, min_freq=1, **kwargs
                    )
    ...
    def create_iterator(self, fields, model_path=None):
        ...
        if not self._train_path_is_name():
            ...
            dataset_cls = next(
                (s for s in [Multi30k, IWSLT, WMT14] if s.__name__ == self._train_path),
                None,
            )
            if dataset_cls is None:
                raise ValueError(
                    "The specified train path {:s}(+{:s}/{:s}) does neither point to a valid files path nor is a name of torchtext dataset class.".format(
                        self._train_path, *self._language_tuple
                    )
                )
            src_suffix, trg_suffix = ext = self._language_tuple
            #      print(ext, fields)
            self._train_data, self._valid_data, self._eval_data = dataset_cls.splits(
                exts=ext, fields=fields
            )  # , split_ratio=self._option.get("train_test_split", const.DEFAULT_TRAIN_TEST_SPLIT)
        else:
            # create dataset from path. Also add all necessary constraints (e.g lengths trimming/excluding)
            src_suffix, trg_suffix = ext = self._language_tuple
            filter_fn = self.create_length_constraint(
                self._option.get("train_max_length", const.DEFAULT_TRAIN_MAX_LENGTH)
            )
            self._train_data = TranslationDataset(
                self._train_path, ext, fields, filter_pred=filter_fn
            )
            self._valid_data = self._eval_data = TranslationDataset(
                self._eval_path, ext, fields
            )
        ...
        self.build_vocab(
            fields, data=train_data, model_path=model_path, **build_vocab_kwargs
        )
        ...
        eval_iter = BucketIterator(
            eval_data,
            batch_size=self._option.get(
